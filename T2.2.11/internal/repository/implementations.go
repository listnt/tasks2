...
func (rep * repository) UpdateEvent(user_id int,date string,event string,newValue models.Event) error{
	query:=fmt.Sprintf("with cte as (select * from events where user_id=%d and date='%s' and event='%s' limit 1) update events s set date='%s',event='%s',description='%s' from cte where cte.user_id=s.user_id",
